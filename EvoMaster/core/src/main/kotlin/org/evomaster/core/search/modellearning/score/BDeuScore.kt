...
/**
 * This class implements the Bayesian Dirichlet likelihood-equivalent uniform joint distribution scoring function.
 * It extends the BDScoring class.
 * It is based on the following article:
 * https://dl.acm.org/doi/10.5555/2100662.2100669
 *
 ...
 */
class BDeuScore(mVariables: Int, rValues: Int): BDScoring(mVariables, rValues) {
    ...
    override fun score(parentMatrix: Array<IntArray>, data: Array<IntArray>): Double {
        ...
        for (i in 0 until mVariables) {
            totalScore += calcPartialScoreG(i, parentMatrix, configsQ[i], data)
        }
    ...
    private fun calcPartialScoreG(i: Int, parentMatrix: Array<IntArray>, confQi: Int, data: Array<IntArray>): Double {
        ...
        for (j in 0 until confQi) {
            ...
            for (k in 0 until rValues) {
                ...
                partialScore += ln(gamma(aijk + Nijk) / gammaAijk)
                Nij += Nijk
            }
