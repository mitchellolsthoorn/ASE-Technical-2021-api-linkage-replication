...
abstract class Mutator<T> : TrackOperator where T : Individual {
    ...
    fun fastMutateAndSave(upToNTimes: Int, individual: EvaluatedIndividual<T>, archive: Archive<T>)
            : EvaluatedIndividual<T>? {
        ...
        for (i in 0 until upToNTimes) {
            ...
            current = EvaluatedIndividual(
                    individual.fitness.copy(),
                    mutatedInd,
                    mutableListOf(),
                    individual.trackOperator
            )
            update(trackedCurrent, current, mutatedGenes)
        }

        return ff.calculateCoverage(current.individual)
                ?.also { archive.addIfNeeded(it) }
    }
}