...
//--- properties
    ...
    @Cfg("Specify a model to use during crossover. If it is set to NONE single point crossover is used.")
    var crossoverModel = CrossoverModel.GOMEA

    @Cfg("Specify the number of iterations to use for learning the model")
    var modelIterations: Int = 50

    @Cfg("Specify the population for the model learning algorithms")
    var modelPopulationSize: Int = 50

    @Cfg("If applicable specify a scoring function to evaluate the model with.")
    var modelScoringFunction = ModelScoringFunction.BDeu

    @Cfg("If applicable specify after how many generations the model should be recalculated.")
    @Min(1.0)
    var recalculationInterval = 1

    @Cfg("If applicable specify with what probability crossover should occur.")
    @Probability
    @Min(0.0)
    @Max(1.0)
    var crossoverProbability = 0.80

    @Experimental
    @Cfg("Specify the current run of the experiment")
    var run = 0
