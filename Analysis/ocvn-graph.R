...
for(alg_pattern in alg_patterns) {
  ...
  for(i in seq(1, 20)) {
    ...
    file_path <- fs::path('ocvn', file_name)
  ...
  ocvn_data <- rbind(ocvn_data, alg_median_data)
}

ocvn_data2 <- dplyr::filter(ocvn_data, mod(generation, 10) == 0)

ggplot2::ggplot(ocvn_data2, aes(x=time, y=coveredTargets)) +
#  geom_point(size=0.5) +
...
ggsave("ocvn.pdf", width = 12, height = 8, units = "cm")
